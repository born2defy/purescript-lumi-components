...
import React.Basic.Hooks (type (/\), Hook, ReactComponent, Ref, UnsafeReference(..), UseEffect, UseLazy, UseRef, UseState, coerceHook, element, useEffect, useLazy, useRef, useState, (/\))
...
useQRCode level value = coerceHook React.do
  ref <- useRef Nullable.null
  qrcode <-
      useLazy (value /\ level) \_ ->
